...
#####
#previous version threw error when best model = intercept only model
#in this version intercept-only-models work as well and report "intercept model -> no effect sizes"
...
manova_AIC_selection<-function(RESPONSE_Mat,MODEL_FULL,n=NULL) {
  ...
  effect_1_1<-effect_1$` Response 1`$Eta2_partial+effect_1$` Response 2`$Eta2_partial #calculate  effect size for the terms 
  
  if(length(effect_1_1)==0)
  {effect_1<-effect_1$` Response 1`$Eta2+effect_1$` Response 2`$Eta2 #calculate  effect size for the terms 
  } else {effect_1<-effect_1_1}
  
  # order after effect size 
  variable_effect<-cbind(MODEL_FULL[["row.names"]][-length(MODEL_FULL[["row.names"]])],effect_1)
  
  if(nrow(variable_effect)==1){variable_effect_sorted<-variable_effect}else{
  ...
  n_u <-if (n>nrow(as.data.frame(variable_effect_sorted))) {nrow(variable_effect_sorted)} else {n} #if one would put in a larger number than terms present in the model the number of model terms in the input model are used 
  ...
  if(nrow(variable_effect_best)==1) {variable_effect_sorted_best<-variable_effect_best
  }else{
